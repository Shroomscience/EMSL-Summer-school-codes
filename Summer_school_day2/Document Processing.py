...
## Load the document
doc = PyPDFLoader("../data/Disappearing Teaspoons.pdf").load()
...
question = "What are the overall findings of the paper?"
for chunk in llm_chain.stream({"question": question, "document": doc}):
  print(chunk, end="", flush=True)
...
question = "How many spoons did they track?"
for chunk in llm_chain.stream({"question": question, "document": doc}):
  print(chunk, end="", flush=True)
...
doc = TextLoader(Path("../data/feynman problems to solve.txt")).load()
question = "How does Feynman advise his friend to pursue a good career?"
for chunk in llm_chain.stream({"question": question, "document": doc}):
  print(chunk, end="", flush=True)
